...
impl DocumentService {
    ...
    pub async fn upload_document(
        ...
        let filename = format!("patient_documents/{}/{}.{}", patient_id, file_id, file_ext);
        
        // Upload to Supabase storage
        let path = format!("/storage/v1/object/patient_documents/{}", filename);
    ...
    pub async fn delete_document(
        ...
        if let Some(filename) = doc.file_url.split("patient_documents/").nth(1) {
            // Delete from storage
            let storage_path = format!("/storage/v1/object/patient_documents/{}", filename);
